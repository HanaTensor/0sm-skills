...
\begin{abstract}
The Einstein field equation is conventionally interpreted as a dynamical law equating spacetime curvature with the stress--energy tensor of matter. In this work, we revisit an alternative perspective that has repeatedly appeared throughout the history of theoretical physics: whether curvature or the stress--energy tensor are truly fundamental, or whether the underlying structure is instead encoded in connections and their associated conservation properties. We review historical attempts by Einstein, Weyl, Cartan, Palatini, and Ashtekar to reformulate gravity in geometric terms, identify the precise point at which these approaches encountered conceptual or empirical limitations, and clarify the nature of the unresolved problem. Building on recent results within the 0-Sphere model, we argue that conservation laws may be understood as consequences of connection-based phase accumulation along thermal geodesic, with stress--energy emerging as a derived, rather than fundamental, quantity.
\end{abstract}

% 30-1で行っている再定式化の核心は、一般相対論における概念の序列を入れ替えることにある。通常の理解では、曲率が基本量であり、Einstein 方程式が力学法則として仮定され、その結果として Bianchi 恒等式が成り立ち、さらにエネルギー運動量保存が導かれると解釈される。しかし本稿では、物理的に意味を持つ基本量を局所的な曲率や力ではなく、固有時に沿った接続の線積分、すなわち holonomy として捉える。この立場では、曲率は接続の非可換性を無限小極限で表した派生量に過ぎず、原理的地位から降格される。Bianchi 恒等式は単なる数学的恒等式ではなく、接続の線積分が閉曲線に沿って一貫して定義されるための整合性条件として再解釈される。したがって、エネルギー保存則は特定の場の方程式を仮定した結果として得られるものではなく、線積分構造を物理的に採用した瞬間に必然的に成立する幾何学的帰結となる。この観点からは、Einstein 方程式自体も原理ではなく、接続の holonomy 構造を局所的な微分方程式として表現した有効記述に位置づけられる。

% [Zenodo登録用]
% [30 Essential] This manuscript proposes a foundational reinterpretation of general relativity based on the premise that physically meaningful quantities are not local tensor fields but line integrals of the spacetime connection evaluated along proper time, i.e., holonomy. Within this perspective, curvature and local field equations are regarded as derived, infinitesimal descriptions of an underlying integral structure rather than as primary objects. The Bianchi identity is reinterpreted not as a subsidiary mathematical identity tied to a specific field equation, but as a fundamental consistency condition required for the global coherence of connection-based line integrals. As a consequence, energy–momentum conservation emerges as a geometric necessity intrinsic to the holonomy framework, rather than as a byproduct of assuming the Einstein field equations. In this formulation, the Einstein equations are naturally demoted to an effective local representation of a more fundamental, nonlocal geometric structure, without being modified or contradicted. The approach preserves all empirical successes of general relativity while reordering its conceptual foundations, shifting the emphasis from curvature-based locality to connection-based integrality.
...
This observation highlights a longstanding conceptual issue: curvature and the stress--energy tensor may not be fundamental, but rather secondary constructs potentially derivable from a more primitive geometric framework. Variants of this perspective have guided multiple reformulation attempts throughout the twentieth century. Despite their conceptual depth, none of these approaches succeeded in replacing the Einstein equation with a fully satisfactory alternative. Variants of this question have guided multiple reformulation attempts throughout the twentieth century.
...
This paper proposes a reformulation of gravitational dynamics in which connection-based geometric structures, rather than curvature or externally imposed energy--momentum sources, play the primary role. 
Section~\ref{sec:historical} reviews historical attempts to recenter gravity around geometric variables beyond the metric, including connection-based and affine formulations, and identifies their common limitations. 
Section~\ref{sec:core_problem} isolates the unresolved conceptual issue shared by these approaches, namely the absence of a geometric mechanism producing experimentally meaningful conservation laws. 
In Section~\ref{sec:0sphere_conservation}, we introduce the present framework, where accumulated connection, phase, and proper-time structure define an internal consistency condition associated with energy conservation.  
We conclude in Section~\ref{sec:conclusion} with a discussion of conceptual implications and future directions.
...
\begin{flushleft}
\footnotesize
\textbf{Note:} The comparison emphasizes differences in the choice of fundamental geometric variables and the points at which each approach encounters conceptual or empirical limitations. 
In contrast to earlier formulations, the 0-Sphere model treats energy conservation as an emergent geometric constraint arising from accumulated connection and proper-time structure, rather than as an externally imposed source term.
\end{flushleft}
\end{table*}
...
To clarify the structural position of the present approach within this historical line of development, Table~\ref{tab:historical_comparison_geometry} summarizes several representative attempts to recenter gravitational theory around geometric variables beyond curvature alone. 
Rather than evaluating these approaches in terms of success or failure, the comparison highlights the choice of fundamental variables and the specific points at which each formulation encountered conceptual or empirical limitations. 
A common pattern emerges: while connections, holonomies, or affine structures were repeatedly identified as more primitive than the metric itself, energy--momentum conservation and experimentally accessible observables remained external inputs or were recovered only indirectly. 
The present formulation differs in that energy conservation is treated as an internal geometric consistency condition arising from accumulated connection and proper-time structure, allowing the Einstein field equation to be recovered a posteriori as a consistency check rather than postulated as a fundamental dynamical law.
...
% failure は感情語ではない。「理論的課題を未解決のまま残した」という 技術的・構造的 failure です。実際、レビュー論文でも failure to explain…, failure to recover the correct limit… は頻出します。また、the historical approaches → these にすることで「一般的失敗」ではなく「本稿で検討した範囲内の評価」になる。(2025/12/22 GPT5.2)

% 直訳調でなく、論文コメントとして自然な訳にすると、次のようになります。『これは、物理的に意味を持つ量が空間上の点ではなく、運動の履歴そのものに結び付いている場合には、局所的な曲率に基づく記述には原理的な限界があることを示している。』もう少し噛み砕くと：『物理量が「その瞬間その点」で決まるのではなく、「どのような経路をたどったか」に依存するなら、局所的な曲率テンソルだけを基本にした記述は十分ではない、という点がここで明らかになる。』

% 物理量の性格によっては、曲率ベースの記述は“主役”になれない場合があると言っている。円柱と蟻の比喩を明記する必要はない。

% 『histories = 世界線・履歴・固有時に沿った運動』この文の意味は：「物理量が“どこにいるか”ではなく“どこを通ってきたか”に結びついているなら、局所曲率を基本にする理論には原理的な限界がある』という表現にとどめる。物理学者ならアリと円柱の違和感は黙示的に知っている。しかし、その違和感を理論の基礎に据える人はほとんどいない。この論文は『黙示的に共有されていた違和感を、明示的な原理へと引き上げている』。だからこそこの穏便な表現が意味を持つ。

% 実は、似た話は別の文脈では主役になっている。皮肉なことに、以下では同じ構造が主役です：
% アハラノフ＝ボーム効果 → 曲率ゼロでも位相が変わる
% ベリー位相 → 局所的ハミルトニアンより履歴が重要
% ウィルソンループ → 物理量は線積分
% しかしこれらは：「量子特有」「ゲージ特有」「非古典的」と分類され、重力には持ち込まれにくい。


\section{Connection-Based Conservation\\ in the 0-Sphere Model}
\label{sec:0sphere_conservation}

Recent work within the 0-Sphere model offers a concrete realization of how conservation laws may emerge from connection-based geometry \cite{hanamura18064535,hanamura17765017}. In this framework, energy transport between localized kernels is governed by a variational principle formulated in terms of
particle-wise proper time $\tau$ \cite{Stueckelberg1941},
leading to what is termed a thermal geodesic.
...
In the present work, this formulation is extended to photon spheres, allowing the internal energy distribution to determine the trajectory of the energy center through the thermal metric.

In homogeneous regions, the thermal geodesic reduces to a straight trajectory in real space. Importantly, this geometric simplicity does not imply trivial internal dynamics. Along the same worldline, relativistic kinematics induce a spinorial connection through Thomas precession \cite{Thomas1926,Jackson}. 
The physically relevant quantity is not curvature integrated over a surface,
but the line integral of the corresponding connection one-form
along the worldline parametrized by proper time,
\begin{equation}
\Phi = \int_{\gamma} \omega .
\end{equation}
% は「ω が内部自由度に作用する接続 1 形式」
...
\Phi = \int_{\gamma} \omega ,
\end{equation}
where $\gamma$ denotes a worldline parametrized by proper time $\tau$, and $\omega$ is a connection one-form acting on internal degrees of freedom.
This expression encapsulates a shift in perspective: physical observables are not associated with local curvature tensors or instantaneous field values, but with the cumulative effect of parallel transport along a physically realized trajectory.

The curve $\gamma$ is not an abstract mathematical path, but corresponds to the actual spacetime history of a localized excitation or kernel evolving along a thermal geodesic.
Proper time provides a natural, observer-independent parameter along this trajectory, ensuring that the accumulated phase is invariant under reparametrization and coordinate transformations.
As a consequence, $\Phi$ is intrinsically tied to measurable clock readings rather than to arbitrary coordinate time.

It is crucial to emphasize that the thermal geodesics considered here are generally \textbf{open curves}, rather than closed loops.
This distinction underpins the operational definition of energy and momentum: physical observables correspond to cumulative geometric effects along the actual trajectories of the centers of mass of localized photon spheres, rather than to abstract field excitations.
Unlike surface- or loop-based integrals, which probe geometric structure indirectly, open line integrals taken along proper-time histories directly encode quantities accessible to measurement.
In this sense, the reliance on open thermal geodesics represents a fundamental shift in perspective, ensuring that conservation laws emerge as intrinsic geometric constraints rather than as postulated or surface-derived properties.

The connection one-form $\omega$ should be understood as an effective geometric object encoding the infinitesimal transformation of internal states under parallel transport.
In relativistic settings, such a connection arises naturally from kinematic effects, including Thomas precession, and may be represented by a spinorial or Lorentz-algebra-valued one-form evaluated along the worldline.
Importantly, the local value of $\omega$ is generally gauge dependent and does not, by itself, constitute a physical observable; only its accumulated integral along $\gamma$ acquires invariant significance.
% 本稿の文脈では線積分に焦点を当てている。「瞬間値は意味がないが、線積分（積分値）には物理的意味がある」という含みを意図的に残している。誤解は生まない。
% ほとんどの文献では、瞬間値を局所物理量として議論する傾向がありますが、本稿は「物理量は世界線に沿った累積量として現れる」という視点を強調するため、敢えて瞬間値の無意味さを前面に出しています。
% これにより、線積分に意味があることを暗示し、読者に「物理量は世界線に沿った構造として理解されるべき」という本稿独自の考え方を印象づけています。
% 結論として、この表現は本稿の論旨（線積分に物理的意味がある）を前提とした、意図的かつ特徴的な書き方です。読み手に「積分の重要性」を自然に認識させる効果があります。(2025/12/22 GPT5.2)

% また、ゲージ依存」というのは、瞬間的な値は座標や基準の取り方（ゲージ）によって自由に変わる、という意味です。物理的に直接測れる量ではない、ということです。
...
The accumulated phase $\Phi$ plays a role analogous to geometric phases familiar from quantum mechanics, including Berry and Aharonov--Bohm phases~\cite{Berry1984}, while differing in a crucial respect: the integration parameter is proper time, and the connection is induced by spacetime geometry and kinematics rather than by an externally prescribed gauge field. 
As a result, the phase accumulation reflects intrinsic geometric properties of motion rather than environmental or background-dependent effects.

Within the 0-Sphere model, energy conservation is encoded in the requirement that phase accumulation along admissible trajectories remain globally consistent. 
Discontinuities or ambiguities in $\Phi$ would signal a breakdown of the underlying geometric structure and, consequently, of the physical description. 
In this sense, conserved energy is not introduced as an independent postulate but emerges as a condition ensuring the coherence of accumulated phase along the thermal geodesic.

This viewpoint clarifies why curvature-based quantities play a secondary role in the present framework. 
Curvature characterizes the noncommutativity of parallel transport around closed loops and is naturally associated with surface integrals. 
However, physical particles trace worldlines rather than surfaces, and experimentally accessible quantities are accumulated along such lines. 
The connection integral therefore constitutes the primary geometric observable, with curvature appearing only as a derived descriptor of consistency across families of trajectories.

From this perspective, the Einstein field equation is reinterpreted as an a posteriori consistency condition. 
Rather than generating dynamics directly, it ensures that the emergent stress--energy tensor inferred from phase accumulation is compatible with the global structure of spacetime connections. 
The equation thus functions as a \textbf{geometric checksum,} verifying that the \textbf{accumulated connection-based description} of energy transport remains \textbf{self-consistent.}
...
The historical development of gravitational theory reveals a persistent insight: curvature and the stress--energy tensor may not be fundamental. 
Although multiple reformulations elevated connections and geometry, none succeeded in deriving conservation laws as direct, measurable consequences of geometric structure.

The 0-Sphere model shows that this remaining step can be accomplished by considering the geometric phase accumulated along connections traced by the thermal geodesic. 
In this framework, conservation laws emerge naturally, and the stress--energy tensor appears as an effective descriptor rather than a fundamental input.
...
% ネーターの定理を所与の公理ではなく創発現象として再定式化。対称性が電子の内部幾何学的構造から生じることを示し、Einstein方程式とネーターの定理を接続積分に基づく統一的枠組みで再解釈する。
...
% 相対論的粒子力学において固有時を独立な力学パラメータとして導入した初期の原論文の一つ。本論文で用いている particle-wise proper time による定式化の歴史的起源を与える参照として引用。固有時を「観測者とは独立な粒子固有の時間」として扱う立場を明確に裏付ける。
...
% 0-Sphereモデルにおける熱測地線の定式化。一般相対性理論を内部熱力学構造を通じて拡張し、粒子の運動を測地線と熱力学的過程の統一として記述する理論的枠組みを提供する。
...
Zenodo (2025).
\href{https://doi.org/10.5281/zenodo.17982104}{https://doi.org/10.5281/zenodo.17982104}
% 不確定性原理への決定論的挑戦。測定を波動関数の崩壊ではなく内部位相の顕在化として再解釈。エネルギー保存恒等式E₀=[cos⁴θ+sin⁴θ+(1/2)sin²(2θ)]とブリッジ方程式γ=1+aを用いて8つの未解決問題を体系的に解決。
...
% U(1)ゲージ対称性を電子の内部時計の同期から導出。粒子中心的な離散ファイバー束を用いてゲージポテンシャルA_μを幾何学的同期機構として再解釈。階層的周期性(ωt/2でスピン4π、ωtでゲージ2π)を単一の時計から統一的に説明。
...
``A Noetherian Inversion: From Einsteinian Geometry to Emergent Symmetry,''
Zenodo (2025).
\href{https://doi.org/10.5281/zenodo.17765244}{https://doi.org/10.5281/zenodo.17765244}
% ネーターの定理の逆転：対称性を所与の公理ではなく創発現象として再定式化。保存則が電子の内部幾何学的構造から生じることを示し、Einstein方程式とネーターの定理を接続積分に基づく統一的枠組みで再解釈する。
...
% 0-Sphereモデルにおける時間依存質量のハミルトニアン定式化。質量変調m(t)=m₀[1-(1/2)sin²(ωt)]のもとで運動量保存p=m(t)ẋ=constantが維持されることを示し、エネルギー保存が内部振動構造を通じて保たれることを解析的に証明する。

\bibitem{Berry1984}
M.~V.~Berry,
``Quantal phase factors accompanying adiabatic changes,''
Proc.\ R.\ Soc.\ Lond.\ A {\bf 392}, 45--57 (1984).
% 断熱変化に伴う幾何位相（Berry位相）を最初に定式化した原論文。
% 本論文で言及している「幾何位相」の概念的起点として引用。
% 可換（U(1)）の場合の標準的参照であり、接続の線積分という
% 幾何学的解釈の基礎を与える。
