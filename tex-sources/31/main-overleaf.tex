...
\usepackage{caption}
\captionsetup[figure]{
    justification=justified,  % 両端揃えに変更
    labelfont={bf},
    name={Fig.},
    labelsep=period
}
...
% [Zenodo登録用]
% [31 Essential]
% A wide range of physical phenomena—including the Aharonov–Bohm effect, Berry phase, Wilson loops, and energy transport in the 0-Sphere model—demonstrate that experimentally observable quantities often depend not on local field values or curvature alone, but on effects accumulated along trajectories. This work articulates a unifying principle: physical observables are fundamentally line integrals of connections, or equivalently, holonomies and accumulated geometric phases defined along histories.

%This paper constitutes the third part of a conceptual trilogy. The first part, Geometric Structure of Spinorial Phase Accumulation along Thermal Geodesics (Zenodo, 2025, https://doi.org/10.5281/zenodo.18067760), established why line integrals are the physically appropriate geometric objects in the 0-Sphere framework, based on the one-dimensional structure of energy transport between localized kernels. The second part, From Curvature to Connection: Revisiting the Geometric Origin of Conservation Laws (Zenodo, 2026, https://doi.org/10.5281/zenodo.18135855), showed that conservation laws themselves emerge as geometric consistency conditions associated with accumulated connection and proper-time structure, reinterpreting the stress–energy tensor as a derived descriptor rather than a fundamental source.

%Building on these results, the present work advances a broader synthesis. By examining canonical examples from quantum mechanics and gauge theory—the Aharonov–Bohm effect, Berry phase, and Wilson loops—alongside the explicit thermal dynamics of the 0-Sphere two-kernel system, we argue that line integrals constitute a natural and universal class of fundamental observables across quantum, gauge, and gravitational domains. Local tensors, field strengths, and curvature are shown to play a secondary role, encoding infinitesimal or loop-level properties of parallel transport, while the operationally meaningful content resides in accumulated effects along physically realized trajectories.

%This perspective does not modify existing theories or their empirical predictions. Instead, it clarifies their logical hierarchy by placing histories, connections, and line-integral observables at the foundational level, with locality and curvature emerging as effective descriptors. In doing so, the work provides a coherent conceptual bridge linking interference phenomena, gauge invariance, and gravitational dynamics under a single geometric principle centered on line integrals as fundamental observables.
...
% REM:
% 本節で用いている線積分および経路長に関する連続的表現は、
% 観測的・有効理論としての coarse-grained 記述を意図したものであり、0-Sphere model における内部構造の離散性（2点間距離＝サーマルポテンシャルエネルギー、カーネル間距離）を仮定的に否定するものではない。
%
% 本モデルでは、離散的な内部カーネル構造に基づく位相応答が、
% 粒子の履歴に沿って累積した結果として、連続的な位相積分として観測される。したがって、連続的な path-length accumulation は内部自由度の連続性を主張するものではなく、離散内部構造の観測可能極限における有効表現である。
%
% 本論文では、線積分を第一義的な観測量として位置づける原理的整理を主目的とするため、内部離散構造と連続積分表現の対応関係については本文では明示的に展開しない。この点は、将来のモデル依存的議論に委ねる。(2026/01/07, GPT）
...
% 物理学者向けに接続・並行移動・ホロノミー・ゲージ束を統一的に解説した標準的参照。本論文で用いる線積分・ホロノミー・Wilsonループ等の幾何学的概念の一般的背景として引用。Kobayashi–Nomizu を呼ばなくて済む立場を明示。「定理」ではなく「概念的背景」としての引用。「点 vs 経路」「局所量 vs 大域構造」という対比は、Nakahara が暗黙の共通知識。
...
% 一般相対論の標準的教科書。Einstein方程式および（縮約）Bianchi恒等式の幾何学的意味と保存則との関係を体系的に整理している。本論文における「局所曲率に基づく記述」との対比の基準点として引用。
