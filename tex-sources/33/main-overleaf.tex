...
% 上記(H(x,1) = p)は deformation retraction の定義文で、D 内の すべての点 x がパラメータ t=1 において同一の点p に写されるという意味です。つまり：空間D 全体が、連続的変形によって一点 p に縮められるという位相的事実だけを述べています。
...
\begin{figure*}[t!] % figure*環境（2カラムまたがり）
    \centering 
    \resizebox{\textwidth}{!}{% !で縦横比を維持
        \input{fig_TotalHamiltonian.tex}%
    }
