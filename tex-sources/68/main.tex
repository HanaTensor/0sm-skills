...
\paragraph{Author decision, 25 July 2026: the refined velocity is rejected.} The status of $0.040374c$ is settled here rather than left as a pending correction. It is \emph{rejected}, not merely withdrawn from the papers that carry it: the corpus recognises one internal velocity, Paper~\#10's $0.04047c$, derived from the anomalous magnetic moment alone. The refined figure, the lepton critical radii, and the decay account built on them are removed from the record of what this programme claims. The decision costs nothing forward: no live result depends on the rejected value, since \#51, \#61, \#62 and~\#67 all use the canonical one.

Rejection is not uniform in what it takes from each paper, and Table~\ref{tab:reject_disposition} states the difference. Two rows deserve emphasis in opposite directions. Paper~\#23 loses a \emph{conclusion}: it argued from the refinement that general relativistic corrections, ``typically considered negligible at the quantum scale, produce observable effects,'' and Paper~\#28 establishes the opposite --- at these scales geodetic precession is of order $10^{-29}$~rad. That inference is not merely unsupported but inverted, and should not be repeated. Paper~\#15, by contrast, loses only a sentence: the bridge equation $\gamma = 1 + a$, which its own abstract names as the study's key contribution, is independent of the geodetic route and stands.
...
\bottomrule
\end{tabular}
\caption{\textbf{What the rejection of $0.040374c$ takes from each affected paper, and what it leaves}}
\label{tab:reject_disposition}
\end{table}
...
\item Lepton mass hierarchy: earlier claims regarding critical radii (Paper~\#14) relied on inserting the mass in kilograms where the geometrized mass belongs, and are \emph{rejected} by author decision of 25 July 2026 (Paper~\#28 supplies the correction) together with the lepton-decay account and the ``geodetically refined'' velocity $0.040374c$ that rested on them; Papers~\#15, \#17, \#20, \#21, \#22, \#23, \#27 and~\#37 inherit these figures or the account, and Table~\ref{tab:reject_disposition} states what the rejection takes from each and what it leaves; the announced stability-origin theme (equal vs.\ unequal internal frequencies) is a research direction, not an established result.
\end{itemize}
