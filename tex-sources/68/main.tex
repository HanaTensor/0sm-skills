...
\addlinespace[0.2cm]
\#17 & $v_\mathrm{ZB}=0.040374\,c$ and the kernel radius $3.43\times10^{-25}$~m, both stated as obtained by combining Lorentz contraction with geodetic precession from general relativity & The geodetic route was corrected in \#28: with the $G/c^2$ factors restored, geodetic precession at these scales is negligible ($3Gm_\mu/c^2 = 4.196\times10^{-55}$~m, giving $\Delta\phi_\mathrm{geodetic}\approx1.32\times10^{-29}$~rad at $R\sim10^{-25}$~m), and the critical-radius notion on which the kernel radius rests is eliminated --- by the geodetic estimate and, independently, because Thomas precession cancels its radius dependence. The fractional difference between \#17's velocity and the canonical one is $2.4\times10^{-3}$, whereas the geodetic phase available to an electron at $R=3.43\times10^{-25}$~m is $1.9\times10^{-32}$~rad: short by twenty-nine orders of magnitude even when read, most generously, as a fractional correction. \textbf{The canonical value is \#10's $v_\mathrm{ZB}=0.04047\,c$} ($\beta^2 = 0.00163798087$), derived from the rotational Lorentz contraction of the anomalous magnetic moment alone and independent of geodetic precession; it is the value used in \#51, \#61, \#62 and \#67. \#17's numerical value should not be quoted. \\
\addlinespace[0.2cm]
...
\midrule
N1 & Explicit $T_{\mu\nu}^{(\mathrm{thermal})}$ & Construct the thermal stress--energy tensor from the spherical-harmonic energy distribution $\rho=\sum a_{\ell m}Y_{\ell m}$ on the domain $\mathcal{D}$. Named and deferred twice in \#24. Its $\ell=2$ component is the quadrupole source. & \#24, \#33, \#57 \\
\addlinespace[0.3cm]
