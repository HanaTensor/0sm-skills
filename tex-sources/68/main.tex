...
\paragraph{Vocabulary.} The log distinguishes four dispositions, and they are not interchangeable. A result is \emph{corrected} when an error is fixed and the paper otherwise stands (``Fixed in \#$n$''). It is \emph{retracted}, or equivalently withdrawn, when the author disowns the claim itself; where the claim recurs across several papers this is stated as retracted \emph{corpus-wide}. It is \emph{superseded} when a later derivation replaces it without the earlier statement having been wrong. And it is \emph{set aside by decision} when the author elects not to pursue a line that has been shown neither erroneous nor superseded --- the disposition of open task O22 in Section~\ref{sec:open}. We avoid the word \emph{rejected} throughout: in ordinary usage it describes what a referee does to a submission or what a test does to a hypothesis, and none of the entries below is either of those. Each is an author's decision about the author's own published claim, which is a retraction.
...
\addlinespace[0.2cm]
\#59 & Eq.~(III.3): the ZB-axis locking ratio and, secondarily, the Zitterbewegung angular frequency of Appendix~B & Entry C1 of the deliberation record \#66. With $g$ the terrestrial acceleration, as the \#52-derived asymmetry term implies, the locking ratio is $\approx9.2\times10^{21}$, not $\approx10^{39}$; the quoted $2.27\times10^{39}$ requires $g = Gm_p/a_0^2$, under which both terms point along $\hat r$ and the comparison becomes self-directional. Appendix~B's $\omega_\mathrm{ZB}\approx1.55\times10^{20}$~rad\,s$^{-1}$ matches neither the Dirac $2m_ec^2/\hbar\approx1.55\times10^{21}$ nor the series value $2\pi\nu_\mathrm{ZB}\approx3.1\times10^{19}$. Confirmed by both models of the session; the qualitative conclusion, Coulomb locking of the ZB axis, is unaffected at $10^{22}$. \\
\addlinespace[0.2cm]
\#58 & Chirality passage: $da/dt = \omega_\mathrm{ZB}\cos\theta$ treats $a$ as an oscillating coordinate & Entry C2 of \#66. Paper~\#50 defines $a$ as the \emph{monotonic} internal phase on $S^1$, which is the basis of the Lorentz invariance of $\chi$; on those definitions $\chi=+1$ is well defined at both ejection points. The root is a collision of notation: the symbol $a$ carries at least three meanings across the corpus --- the monotonic internal phase (\#50), a kinematic convention (\#10, \#49), and the anomalous magnetic moment. Confirmed by both models. \\
\addlinespace[0.2cm]
\#58 & The $\theta=\pi/2$ kinematics and the $\chi$-degeneracy reading of the ejection point & Entry C3 of \#66. The same phase point is assigned both $E_\mathrm{ph}(\pi/2)=E_0/2$, the maximum of $E_\mathrm{ph}=\tfrac{E_0}{2}\sin^2\theta$, and $\mathbf v\propto\cos(\pi/2)=0$, a turning configuration at rest. Read as a total velocity this is self-contradictory; read as longitudinal only it reduces to C2. Either way the inference that ejection occurs at a degenerate boundary between chirality sectors, and hence that the mediator is its own antiparticle, does not follow; self-conjugacy is available by the photon-sector route at no cost. The root is the \#10 convention ($v=\cos\omega t$, rest at $\pi/2$) against the \#49 convention ($|v|\propto\sin\theta$, maximum at $\pi/2$), which is open task~(viii) of \#49. Both models recommend retracting the degeneracy reading; \textbf{the canonical decision is reserved by the author} and is not made here. Paper~\#60 inherits \#58's turning-point characterization, so the resolution propagates to it. \\
\addlinespace[0.2cm]
...
\paragraph{Author decision, 25 July 2026: the refined velocity is retracted.} The status of $0.040374c$ is settled here rather than left as a pending correction. It is \emph{retracted corpus-wide}, not merely withdrawn from the papers that carry it: the corpus recognises one internal velocity, Paper~\#10's $0.04047c$, derived from the anomalous magnetic moment alone. The refined figure, the lepton critical radii, and the decay account built on them are removed from the record of what this programme claims. The decision costs nothing forward: no live result depends on the retracted value, since \#51, \#61, \#62 and~\#67 all use the canonical one.

It is worth stating what the retraction does \emph{for} the canonical value, because the effect is the opposite of a loss. The internal velocity $\beta_{\mathrm{ZB}} = 0.04047c$ is the programme's first-order empirical claim: a parameter-free number, fixed entirely by the measured anomalous magnetic moment through the bridge equation, and \emph{not yet measured}. Until now the corpus offered two figures for the same observable, and a prediction that comes in two versions is not a prediction --- no measurement could have falsified either cleanly. Retracting one leaves a single number to be tested. The access routes remain those the corpus already identifies: the helicity-flip condition of Section~\ref{sec:zb}, which is what makes the velocity independently testable rather than merely invertible from $a_e$, and the transverse-channel frequency $\nu_\mathrm{ZB} = v_{\mathrm{ZB}}/\lambda_C \approx 5.0\times10^{18}$~Hz, which differs from the Dirac estimate $3.93\times10^{20}$~Hz by a factor of about eighty and which Paper~\#51 connects to trapped-ion quantum simulation of the Dirac equation. This prediction stands, awaiting measurement, and is unaffected by anything retracted here.

Retraction is not uniform in what it takes from each paper, and Table~\ref{tab:retract_disposition} states the difference. Two rows deserve emphasis in opposite directions. Paper~\#23 loses a \emph{conclusion}: it argued from the refinement that general relativistic corrections, ``typically considered negligible at the quantum scale, produce observable effects,'' and Paper~\#28 establishes the opposite --- at these scales geodetic precession is of order $10^{-29}$~rad. That inference is not merely unsupported but inverted, and should not be repeated. Paper~\#15, by contrast, loses only a sentence: the bridge equation $\gamma = 1 + a$, which its own abstract names as the study's key contribution, is independent of the geodetic route and stands.
...
\toprule
\textbf{Paper} & \textbf{Rejected} & \textbf{Unaffected} \\
\midrule
\#14 & The critical radii, the refined velocity derived from them, and the lepton-decay account & The special-relativistic value $0.040472c$, which it also reports \\
\addlinespace[0.2cm]
\#15 & The refined velocity, stated in the abstract & \textbf{The bridge equation $\gamma = 1+a$}, the paper's stated key contribution \\
\addlinespace[0.2cm]
\#17 & The quoted velocity and kernel radius & The construction of $U(1)$ gauge structure from internal clock synchronization \\
\addlinespace[0.2cm]
\#20 & The decay-endpoint explanation and the ``immediate experimental target'' & The Noetherian reinterpretation of conservation laws \\
\addlinespace[0.2cm]
\#21 & The critical-radius explanation of lepton stability and generation structure & The field-free, observer-dependent reading of the anomalous moment \\
\addlinespace[0.2cm]
\#22 & The quoted baseline velocity only & \textbf{The gravitational modulation $\Delta\nu/\nu\approx5.29\times10^{-10}$}, a ratio of clock rates in which the velocity cancels \\
\addlinespace[0.2cm]
\#23 & \textbf{The conclusion} that general relativistic corrections are non-negligible at the quantum scale --- inverted by \#28 & The zero-sphere to emergent-spacetime framework itself \\
\addlinespace[0.2cm]
\#27 & The refined velocity offered as a precise experimental prediction & The deterministic reading of measurement \\
\addlinespace[0.2cm]
\#37 & The proposed lepton critical radii & The line-integral reading of gravitational potential energy \\
\addlinespace[0.2cm]
\bottomrule
\end{tabular}
\caption{\textbf{What the retraction of $0.040374c$ takes from each affected paper, and what it leaves}}
\label{tab:retract_disposition}
\end{table}
...
\item Lepton mass hierarchy: earlier claims regarding critical radii (Paper~\#14) relied on inserting the mass in kilograms where the geometrized mass belongs, and are \emph{retracted} by author decision of 25 July 2026 (Paper~\#28 supplies the correction) together with the lepton-decay account and the ``geodetically refined'' velocity $0.040374c$ that rested on them; Papers~\#15, \#17, \#20, \#21, \#22, \#23, \#27 and~\#37 inherit these figures or the account, and Table~\ref{tab:retract_disposition} states what the retraction takes from each and what it leaves; the announced stability-origin theme (equal vs.\ unequal internal frequencies) is a research direction, not an established result.
\end{itemize}
...
Four tasks replace the registry as the working frontier. They are stated in Section~\ref{sec:restart_programme} and repeated here in registry form. The prefix \textbf{N} denotes the \emph{narrowed} programme of Section~\ref{sec:restart}, and is deliberately a different letter from the \textbf{O} of the registry above: those twenty-two entries are retained as a closed record, and reusing their letter would blur the boundary this section draws. An entry numbered O is part of the retrospective; an entry numbered N is being worked.
