...
\begin{abstract}
Paper~\#52 derived the weak equivalence principle inside the 0-Sphere model from the $\lambdaC$ cancellation theorem, obtaining $F = m g\,\betaZB$ with $\betaZB = \vZB/c \approx 0.04$, and recorded the recovery of the full Newtonian $F = mg$ as an open task.
That derivation established the \emph{mass} side of the principle: the gravitational acceleration does not depend on how heavy the particle is.
The present paper tests the two sides the derivation did not reach, and finds them to behave very differently.

The first test is the bound state.
We ask whether a bound electron gravitates by its reduced total energy, so that the binding-energy mass defect is correctly carried.
Redoing the $\lambdaC$ cancellation for a bound electron gives $F = (E/c^2)\,g\,\betaZB$ exactly, and the Thomas vortex of Paper~\#58 reduces independently to $\OmT = \bigl(\vZB^{3}/4\hbar c^{3}\bigr)E$, strictly linear in the total energy.
The model therefore passes: the binding defect is carried automatically, and it is carried because $\vZB$ is fixed to the anomalous moment by the bridge equation rather than being a dynamical variable.
Two exact statements about Dirac--Coulomb bound states support this.
We prove by Hellmann--Feynman that $\expval{\beta} = E/mc^2$ for every such state, whence the negative-energy admixture equals $B/2mc^2$ --- the Zitterbewegung content of a bound state \emph{is} its binding-energy defect.
Both are verified numerically to machine precision for eighteen state--charge combinations.

The second test is composition, and here the model does not pass as it stands.
Paper~\#61 applies the same bridge equation to the proton and obtains $\vZB^{p} \approx 0.898\,c$, so $\betaZB$ is not a universal number: it is $0.0405$ for leptons and $0.898$ for nucleons, a ratio of $22.2$.
With $F = mg\,\betaZB$ read literally, bulk matter acquires a composition-dependent free fall, $\eta(\mathrm{Ti},\mathrm{Pt}) = 3.13\times10^{-5}$, exceeding the MICROSCOPE bound by $1.2\times10^{10}$.
The conclusion is constructive rather than destructive: the mechanism that completes Paper~\#52 must \emph{cancel} $\betaZB$ rather than rescale it, and this converts a deferred numerical coefficient into a structural requirement.
Any reading in which the gravitational charge is non-linear in energy is excluded still more sharply by the nuclear-binding lever, at $3.2\times10^{11}$.
What survives is a clean division of labour: $\OmT$ carries the spin-2 polarisation structure, and the total energy alone carries the coupling strength.

Separately, we record an incompatibility between Paper~\#62, which asserts the field-independence of $a_e$, and Paper~\#63, whose horizon-freezing prediction requires $\vZB \to 0$; the two cannot both hold locally, and we show that the question is already settled.
The local reading requires a violation of local position invariance with coefficient $c^2 a^{-1}\dd a/\dd\Phi = 2$, whereas atomic clock comparisons bound the corresponding coefficient for $\alpha$ at $14(11)\times10^{-9}$ --- an exclusion by eight orders of magnitude.
Paper~\#63's freezing must therefore be read as a distant-observer statement, which removes it as a point of divergence from general relativity.
Read together, the failures are not four difficulties but one: an internal kinematic quantity, tied by the bridge equation to the anomalous moment and hence to the species, has been asked to carry a charge that must be the total energy alone.
The same property that makes the model pass the bound-state test --- that $\betaZB$ is a pure number --- makes it fail the composition test, because it is a different pure number for each species.
Section~\ref{sec:diagnosis} draws the consequence: the mediator-construction route of Papers~\#34, \#49, \#52 and~\#58 cannot supply a gravitational charge.
The same section reads Paper~\#66's recent advance on the rank-2 question in this light --- its source-side construction succeeds by importing the radiation formalism of general relativity, and the burden it leaves behind sits on the closure edge identifying Berry curvature with the Riemann tensor --- so that representation theory and the equivalence principle point at the same place from opposite directions.
We therefore declare the formalisation restarted from the line-integral series of Papers~\#29, \#30 and~\#31, where stress--energy is a derived quantity read off from accumulated phase rather than a quantity assembled from internal kinematics.
Four acceptance criteria for that restart are stated, two of them supplied by the present tests.
This manuscript is written to be read on its own. Section~\ref{sec:primer} is a self-contained primer on the 0-Sphere model --- what it proposes, where each ingredient used here comes from, and what within the framework is established as against proposed --- so that a reader who has met the Dirac equation but not the series can follow every step without consulting the sixty preceding papers; a reading map identifying the six that are load-bearing is given in Table~\ref{tab:papermap}. Specialists may skip to Section~\ref{sec:bound}.
\end{abstract}
...
\label{eq:bridge}
\end{equation}
For the electron, $\lvert a_e\rvert = 1.15965\times10^{-3}$ gives $\betaZB = 0.04047$, the canonical value of the series. It is worth noting for precision that the corpus contains one other figure: Papers~\#14, \#15, \#17, \#20, \#22, \#23, \#27 and~\#37 quote a ``geodetically refined'' $0.040374\,c$, which rests on a dimensional slip corrected in Paper~\#28 and has been rejected by author decision of 25 July 2026; the canonical value is the one Paper~\#10 derives from the anomalous moment alone, $\beta^2 = 0.00163798087$, and is what Papers~\#51, \#61 and~\#62 use and what is used throughout the present paper. Omitting the $1/\sqrt{2}$ gives the peak value $0.0481$; the residual factor $2^{1/4}$ between the two placements is a recorded open item of Paper~\#62 and does not affect anything in this paper, because both are pure numbers.
...
\begin{tabular}{p{2.2cm}p{7.2cm}p{5.6cm}}
\toprule
...
\addlinespace[0.3cm]
$\gamma_L = 1+\lvert a\rvert$
& mass-determining Lorentz factor of the internal motion
...
& Papers~\#34, \#49 \\
\addlinespace[0.3cm]
$\gammaD = \sqrt{1-(Z\alpha)^{2}}$
& exact Dirac $1s$ energy in units of $mc^{2}$, point nucleus
...
\label{tab:verdict}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{p{3.6cm}p{3.0cm}p{2.2cm}p{3.0cm}p{2.8cm}}
\toprule
...
\addlinespace[0.3cm]
Force law $mg\,\betaZB$
& Paper~\#52, Eq.~\eqref{eq:F52}
& $E^{1}$, $\vZB^{1}$
& species lever, $1.2\times10^{10}$
& completion must cancel $\betaZB$ \\
\addlinespace[0.3cm]
Emission power
& rate $\times$ fragment energy
& $E^{2}$
& nuclear lever, $3.2\times10^{11}$
& excluded \\
\addlinespace[0.3cm]
$\OmT$ with $\vZB \propto E$
& Paper~\#63 horizon reading
& $E^{3}$
...
\label{tab:diagnosis}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{p{4.2cm}p{2.0cm}p{4.4cm}p{4.0cm}}
\toprule
...
\addlinespace[0.3cm]
Charge read as emission power
& exchange picture
& ejection rate $\times$ fragment energy
& non-linear in $E$, excluded by $3.2\times10^{11}$ \\
\addlinespace[0.3cm]
Horizon freezing requires $\vZB \to 0$
& Paper~\#63
& the internal speed responding to $\Phi$
