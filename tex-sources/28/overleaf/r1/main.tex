...
% 要約: QEDの1ループ補正により電子の異常磁気モーメント α/(2π) を計算。量子補正の概念を確立。

\bibitem{dirac1928}
P.~A.~M.~Dirac,
``The Quantum Theory of the Electron,''
\textit{Proc. R. Soc. Lond. A} \textbf{117}, 610 (1928).
% 要約: ディラック方程式を導入し、電子のスピンと反粒子の存在を予言。磁気モーメント g=2 の理論的基礎を確立。
...
% 要約: Zitterbewegung（ジッター運動）の概念を初めて提唱。ディラック方程式における高周波振動現象を記述。
...
% 要約: 史上最高精度での電子の異常磁気モーメントの測定。QEDの検証と新物理の探索に関与。
...
% 要約: レプトンの異常磁気モーメントとZitterbewegung速度の直接的代数的関係を確立し、
% 量子力学現象を特殊・一般相対論的原理と統合。測地歳差運動を考慮した精密な予測値を導出。
...
% 要約: 電子を2つの完全黒体として再定義し、仮想光子を実在の光子として解釈する電子モデルを提案。
% スピノール的振る舞いと自己エネルギー問題の解決を目指す。
