...
\date{September 23, 2025 (Revised: February 13, 2026)}
...
When the correct $G$ and $c^2$ factors are restored, geodetic precession becomes negligible at quantum scales ($\Delta\phi_{\rm geodetic} \sim 10^{-29}$~rad at $R \sim 10^{-25}$~m), 
...
This revision corrects numerical errors in the intermediate quantity $3Gm_\mu/c^2$ and in the geodetic precession estimates that propagated from it. Specifically:
\begin{itemize}
\item The value $3Gm_\mu/c^2 \approx 7.496 \times 10^{-60}$~m stated in v1 is corrected to $\approx 4.196 \times 10^{-55}$~m (the v1 value was too small by a factor of $\sim 5.6 \times 10^4$).
\item The geodetic precession at $R \sim 10^{-25}$~m is corrected from $\sim 2.35 \times 10^{-34}$~rad to $\sim 1.32 \times 10^{-29}$~rad.
\item References to ``$10^{-52}$~rad'' in the abstract and conclusion are corrected to ``$10^{-29}$~rad.''
\end{itemize}
The final result $R_{\rm crit} \approx 2.55 \times 10^{-52}$~m and all qualitative conclusions are unchanged.
...
Applying this to a muon mass with $R$ at the microscopic scale ($R \sim 10^{-25}$~m) yields
\begin{equation}
\Delta \phi_{\mathrm{geodetic}} \sim 10^{-29} \ \ \text{rad per orbit}.
\end{equation}
...
% ----------------------------------------
% Subsection 4.1: Special-relativistic angle deficit
% ----------------------------------------
...
% ----------------------------------------
% Subsection 4.2: Geodetic precession
% ----------------------------------------
...
\begin{equation}
\frac{3GM}{c^{2}} \;=\; \frac{3Gm_{\mu}}{c^{2}} \approx 4.196\times10^{-55}\ \mathrm{m}.
\label{eq:3GM_over_c2}
\end{equation}
Therefore a radius $R$ of order $10^{-25}\,\mathrm{m}$ gives
\begin{align}
\Delta\phi_{\mathrm{geodetic}} &\sim \frac{3\pi G m_\mu}{c^{2} R} \label{eq:geodetic_estimate_a}\\
&\sim \pi\times\frac{4.196\times10^{-55}}{10^{-25}} \label{eq:geodetic_estimate_b}\\
&\sim 1.32\times10^{-29}\ \mathrm{rad}, \label{eq:geodetic_estimate_c}
\end{align}
...
\end{equation}
so the GR-based critical radius with correct dimensions is absurdly small compared to any physically relevant microscopic scale. By contrast, the omission of $G/c^{2}$ (i.e. using $3M/\beta^{2}$ with $M$ in kg) produces a numerical value around $10^{-25}\,\mathrm{m}$ --- this is a units mistake, not a physical effect. The overestimation factor is $c^2/G \approx 1.35 \times 10^{27}$, i.e., approximately 27 orders of magnitude. We document both values explicitly to make this point quantitative.

% ----------------------------------------
% Subsection 4.3: Thomas precession
% ----------------------------------------
...
% ----------------------------------------
% Subsection 4.4: Numerical evaluation
% ----------------------------------------
...
% ----------------------------------------
% Subsection 4.5: Angle-linear composition
% ----------------------------------------
...
% ----------------------------------------
% Subsection 4.7: Gravitomagnetism and semi-classical backreaction
% ----------------------------------------
...
We have revisited the attempt to combine SR and GR corrections in the microscopic dynamics of leptons. Restoring the correct dimensional factors in geodetic precession reduces its contribution to $\sim 10^{-29}$~rad (at $R \sim 10^{-25}$~m), with the dimensionally consistent critical radius evaluating to $R_{\rm crit} \approx 2.55 \times 10^{-52}$~m --- far below any physically relevant scale. The muon critical radius reported in earlier work 
