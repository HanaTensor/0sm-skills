...
% ここでの幾何学的位相は比喩ではなく、Berry (1984) による断熱ホロノミーの定義に対応する。Simon (1983) はこれをファイバー束の言葉で厳密化しており、本モデルの内部 SU(2) 構造が幾何学的位相を持つことの数学的正当性を保証する。
...
% 赤外発散は QED 固有の近代的問題ではなく、Bloch–Nordsieck (1937) により電子が必然的に伴う放射場の問題として早期に認識されていた。本モデルは、この歴史的問題に対し「内部幾何による閉じた放射循環」という別の物理的解釈を与える。

\end{thebibliography}

\section{appendix}

\subsection{Monte Carlo Simulation Code for Ensemble-Averaged Visibility}
...
For a single electron with internal geometric phase $\theta$, the visibility function is
\begin{equation}
