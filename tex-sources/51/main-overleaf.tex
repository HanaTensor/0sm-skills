...
\newcommand{\iphase}{\theta}  % internal Zitterbewegung phase = omega_ZB * t (full ZB phase)
\usepackage{tikz}
...
\date{April 12, 2026; revised May 29, 2026}
...
\midrule
Two-kernel internal structure; energy identity $E_0 = \cos^4\tfrac{\iphase}{2} + \sin^4\tfrac{\iphase}{2} + \tfrac{1}{2}\sin^2\iphase$
...
The physical mechanism that propels the inter-kernel shuttling is the radiation gradient of the kernel potential difference---the driving force $F$ first established in the foundational Paper~\#1~\cite{hanamura01} and carried unchanged through the subsequent development of the model. The two kernels carry the scalar thermal potentials $T_{e1} = E_0\cos^4\tfrac{\iphase}{2}$ for Kernel~A and $T_{e2} = E_0\sin^4\tfrac{\iphase}{2}$ for Kernel~B, where $\iphase = \omZB t$ is the internal Zitterbewegung phase; the quartic kernel terms carry the half-angle $\iphase/2$ because the Dirac spinor returns to its initial state only after a $4\pi$ rotation. Their difference collapses to a single harmonic,
\begin{equation}
T_{e2} - T_{e1} \;=\; E_0\!\left(\sin^4\tfrac{\iphase}{2} - \cos^4\tfrac{\iphase}{2}\right) \;=\; -\,E_0\cos\iphase,
\label{eq:kernel_difference}
\end{equation}
and the radiation gradient of this difference is the driving force:
\begin{equation}
F(\iphase) \;=\; \frac{d}{d\iphase}\!\left(T_{e2} - T_{e1}\right) \;=\; E_0\sin\iphase.
\label{eq:radiation_force}
\end{equation}
Equation~\eqref{eq:radiation_force} is the radiation gradient $\mathrm{grad}(T_{e2} - T_{e1}) = E_0\sin\iphase$ derived in Paper~\#1~\cite{hanamura01}; it is reproduced here verbatim, as the driving term of the helical motion, with no modification. The force alternates in sign with the internal phase: it is positive throughout the Kernel~A~$\to$~Kernel~B traversal ($0 < \iphase < \pi$) and negative throughout the return ($\pi < \iphase < 2\pi$), so a single radiation gradient drives the inter-kernel shuttling in both directions. The photon-sphere kinetic energy is independently bounded below by the floor $E_0/2$ established in the companion paper~\cite{hanamura46}, so the shuttling never terminates at either kernel.

The key observation is that $F(\iphase)$ constitutes a \emph{kinetic energy input} to the helical motion. As the phase advances from $\iphase=0$ (Kernel~A active) to $\iphase=\pi$ (Kernel~B active), the radiation gradient does strictly positive net work, $\int_0^{\pi} F(\iphase)\,d\iphase = E_0\int_0^{\pi}\sin\iphase\,d\iphase = 2E_0 > 0$, transferring energy from the scalar kernel potential into the longitudinal ($x$-direction) kinetic energy of the helix. This is the driving mechanism of the helical propulsion: the sinusoidal radiation gradient converts internal phase dynamics into directed longitudinal motion, without requiring any externally applied force.
...
& Radiation gradient $E_0\sin\iphase$ \\
\addlinespace[0.3cm]
Relation to GR
...
\bibitem{hanamura30}
S.~Hanamura, \textit{From Curvature to Connection: Revisiting the Geometric Origin of Conservation Laws}, Zenodo (2026). \href{https://doi.org/10.5281/zenodo.18819682}{10.5281/zenodo.18819682}
