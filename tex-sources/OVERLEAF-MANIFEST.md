...
- **旧 99-1「Rethinking Particles as Spacetime Oscillators (Research Summary)」は採番せず破棄(2026-07-19)** — コーパスと全面重複のため(User 承認)。**#65 以降(#66–#68 含む)は通常の通番として今後の論文に使用する(欠番にしない)**。**通番を 2026-07-25 に再割当**(いずれも未寄託のため履歴不要・User 承認)。旧割当 #65=総説 / #66=討論記録 / #67=quartic を、まず **#65=quartic / #66=討論記録 / #67=総説** に変更し、同日さらに **#67=等価原理2試験 / #68=総説** へ移管(理由: #67 が #65・#66 を引用して成立するため後方参照とし、総説はバッチ最後に置いて #67 をカタログできるようにする=総説自身の設計意図どおり)。理由:(1) 総説が quartic 論文に実質依存するため、総説を最後に置けば全参照が後方参照になる;(2) 総説が #65・#66 を「in preparation」ではなく正規カタログ行として収録できる;(3) quartic は唯一の一次研究で前方参照ゼロ。**旧マニフェスト記述「#66 が #65 を参照するため」は誤り** — #66 の参考文献は #63 までで総説を引いていない(本文2箇所の番号参照のみ、#67 に更新済み)。
  ...
  - **語彙規約の明文化(2026-07-25)**: 訂正ログは4つの処分を区別する — **corrected**(誤りを修正・論文は存続)／**retracted = 撤回**(著者が主張そのものを取り下げ。複数論文に跨る場合は corpus-wide と明記)／**superseded**(後続の導出が置き換え・元が誤りだったわけではない)／**set aside by decision**(誤りでも被代替でもないが追求しない = O22)。**「rejected/棄却」は使わない** — 通常用法では査読者が投稿を却下する／検定が仮説を棄却する意味であり、著者が自らの公表主張を取り下げる行為は retraction。#68 訂正ログ冒頭に明記。
  - **0.040374c のコーパス全体撤回(2026-07-25 著者判断)**: 「特定論文からの撤回」ではなく**値そのものの corpus-wide 撤回**。コーパスが認める内部速度は #10 の 0.04047c 一つのみ。#68 訂正ログ直後に著者判断の段落＋**棄却内訳表**(tab:reject_disposition、9本×棄却されるもの/残るもの)を新設。**結論レベルの犠牲1件** = #23「GR 補正は量子スケールで無視できず観測可能な効果を生む」→ #28 が示す 10⁻²⁹ rad により**反転**、繰り返し禁止。**過剰棄却の禁止2件** = #22 の Δν/ν≈5.29×10⁻¹⁰(時計比で速度が相殺)、#15 の橋渡し方程式 γ=1+a(#15 自身が「key contribution」と呼ぶもの・測地歳差と無関係)。Scope Boundaries・LLM 注記・#67 §II.C・wall-practice-index・index.md に反映。**加えて 0.04047c の身分を明記** = 異常磁気モーメントのみで固定されるパラメータフリーの**未測定の第一義的予言**。撤回は同一観測量に対する二つ目の数を除いたので**反証可能性を上げる**（二つの版を持つ予言は予言ではない）。検証経路 = helicity-flip 条件・ν_ZB ≈ 5.0e18 Hz（Dirac 3.93e20 Hz と約80倍差）。
  - **#65–#68 の追補(2026-07-25)**: 全数スキャン後の残穴を4件処理。**(1) #68 訂正ログに #66 の C1/C2/C3 を追加**(従来 #58/#59 のエントリがゼロだった。C1=#59 Eq.(III.3) の locking ratio 9.2e21 vs 記載 1e39＋Appendix B の ω_ZB 1.55e20 の不整合／C2=#58 カイラリティ節の記号 a の三重衝突／C3=#58 の θ=π/2 運動学と χ 縮退読み・**正典判断は著者保留**と明記)。**(2) #66 の総説番号参照 2箇所を #67→#68 に修正**(1012行・1151行)。**(3) #65 ヘッダのバッチ記述を #65–#67→#65–#68 に更新**。**(4) #67 §II.C に撤回値の注記を追加**(0.04047 が正準である旨と、0.040374 を持つ8本の列挙)。なお #68 は article クラスで \iphase 未定義のため C2/C3 内を \theta に置換(4箇所)。
  ...
  - コンパイル検証(tectonic・2026-07-25): #65 / #66 / #67 / #68 とも PDF 生成 OK・未定義参照 0・エラー 0(#65 = 202 KB、#66 = 313 KB、#67 = 305 KB、#68 = 357 KB[撤回・語彙規約・予言身分の明記後])。
