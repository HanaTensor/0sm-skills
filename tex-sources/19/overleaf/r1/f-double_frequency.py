...
# Create the plot with larger figure size for A4 printing
...
# Plot velocity
ax1.plot(t, velocity, 'b-', linewidth=3, label=r'$v = \cos(\omega t)$')
ax1.set_ylabel('Velocity', fontsize=18, fontweight='bold')
ax1.grid(True, alpha=0.3)
ax1.legend(fontsize=16)
ax1.set_title('Internal Motion Components in 0-Sphere Model', fontsize=20, fontweight='bold')

# Plot acceleration
ax2.plot(t, acceleration, 'r-', linewidth=3, label=r'$a = -\sin(\omega t)$')
ax2.set_ylabel('Acceleration', fontsize=18, fontweight='bold')
ax2.grid(True, alpha=0.3)
ax2.legend(fontsize=16)

# Plot angular velocity
ax3.plot(t, angular_velocity, 'g-', linewidth=3, label=r'$\Omega = -\frac{1}{4}\sin(2\omega t)$')
ax3.set_ylabel('Angular Velocity', fontsize=18, fontweight='bold')
ax3.set_xlabel(r'Time ($\omega t$)', fontsize=18, fontweight='bold')
...
for i in range(5):
    ...
    if i < 3:
        ax1.axvline(x=i*2*np.pi/omega, color='blue', linestyle='--', alpha=0.5, linewidth=2)
        ax2.axvline(x=i*2*np.pi/omega, color='red', linestyle='--', alpha=0.5, linewidth=2)
    
    # Period markers for Ω (period = π/ω, twice as frequent)
    ax3.axvline(x=i*np.pi/omega, color='green', linestyle='--', alpha=0.5, linewidth=2)

# Set x-axis ticks in terms of π with larger font
...
for ax in [ax1, ax2, ax3]:
    ...
    ax.tick_params(axis='y', labelsize=16)  # Y軸の目盛り文字サイズ

# Add annotations with larger font sizes
ax1.annotate('Period: $2\pi$', xy=(2*np.pi/omega, 0.8), fontsize=14, fontweight='bold',
             bbox=dict(boxstyle="round,pad=0.4", facecolor="lightblue", alpha=0.8))
ax2.annotate('Period: $2\pi$', xy=(2*np.pi/omega, -0.8), fontsize=14, fontweight='bold',
             bbox=dict(boxstyle="round,pad=0.4", facecolor="lightcoral", alpha=0.8))
ax3.annotate('Period: $\pi$ (Double frequency)', xy=(np.pi/omega, -0.2), fontsize=14, fontweight='bold',
