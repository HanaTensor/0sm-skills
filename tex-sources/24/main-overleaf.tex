...
% 要約: 一般相対性理論と電磁気学の古典的理論を詳細に解説し、時空幾何学とエネルギー-運動量テンソルの関係を確立。0-Sphereモデルにおける外部時空との結合の基礎を提供。
...
% 要約: 一般相対性理論の数学的および物理的基礎を詳細に解説。0-Sphereモデルの熱応力-エネルギー・テンソルとアインシュタイン方程式の統合に理論的基盤を提供。
...
% 要約: ディラック方程式の量子シミュレーションを通じてZitterbewegungを観測
...
% 要約: レプトンの異常磁気モーメントとツィッターベーヴェグング速度の直接的代数関係を確立
...
% 要約: Zitterbewegungを物理的実在として再解釈し、電子内部の振動が重力的時間拡張により周波数変調を受けることを予測。内部熱的クロックと時空幾何学の結合機構を詳細に解析し、衛星高度での周波数差を定量的に算出。
...
% 要約: 最小限のS⁰トポロジー（2点の非連結集合）から背景独立的に時空構造が創発することを示し、内部変換からプロパータイム、空間分離、因果性が発生する機構を提案。0-Sphereモデルの背景独立性の概念的基盤を確立。
...
% 要約: 電子スピンと異常磁気モーメントを調和振動とローレンツ収縮を通じて再定義
...
% 要約: 一般相対性理論の包括的教科書であり、測地線と時空曲率の幾何学的解釈を詳細に解説。0-Sphereモデルの測地線拡張の基礎を提供。
...
% 要約: 非平衡熱力学における自由エネルギーの変動定理を提案。0-Sphereモデルの熱測地線と熱力学的アクションの定式化に理論的裏付けを提供。
...
% 要約: ディラック方程式を導入し、電子のスピンとZitterbewegungを初めて記述
...
% 要約: 電子の内部構造とZitterbewegungの幾何学的解釈を提案
...
% 要約: ボース・アインシュタイン凝縮体でのZitterbewegungの直接観測
...
% 要約: フォトンスフィアの内部運動からスピンとゲージ対称性の幾何学的起源を導出
